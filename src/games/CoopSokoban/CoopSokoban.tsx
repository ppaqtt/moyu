...
import { CoopSokobanEngine, Position } from './engine';
import { COOP_SOKOBAN_CONSTANTS, NEON_COLORS } from '../../utils/constants';

const { CELL_SIZE } = COOP_SOKOBAN_CONSTANTS;

export default function CoopSokoban() {
  const navigate = useNavigate();
  const engineRef = useRef<CoopSokobanEngine | null>(null);
  const [state, setState] = useState(() => {
    engineRef.current = new CoopSokobanEngine();
    return engineRef.current.getState();
  });
  const [message, setMessage] = useState('双人合作推箱子!');
  ...
  const boardWidth = state.walls.length > 0 ? 
    Math.max(...state.walls.map(w => w.x)) + 1 : 8;
  const boardHeight = state.walls.length > 0 ? 
    Math.max(...state.walls.map(w => w.y)) + 1 : 8;
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4"
      ...
      >
        ...
        <p className="text-gray-400">合作推箱子到目标点!</p>
      </motion.div>
      ...
      >
        ...
        <div className="glass-card px-6 py-3 rounded-xl text-center">
          <div className="text-sm text-gray-400">箱子</div>
          <div className="text-2xl font-bold text-orange-400">
            {state.boxes.filter(b => b.onTarget).length} / {state.boxes.length}
          </div>
        </div>
      </motion.div>
        ...
        <div className="glass-card rounded-2xl p-4">
          ...
          >
            ...
            {state.boxes.map((box, i) => (
              ...
              >
                <div
                  className={`rounded-lg flex items-center justify-center text-xl shadow-lg transition-colors ${
                    box.onTarget ? 'bg-green-500' : 'bg-orange-500'
                  }`}
                  style={{
                    width: CELL_SIZE - 8,
                    height: CELL_SIZE - 8,
                    boxShadow: box.onTarget ? '0 0 20px rgba(34, 197, 94, 0.5)' : '0 4px 8px rgba(0, 0, 0, 0.3)'
                  }}
                >
                  📦
                </div>
            ...
            {state.players.map(player => (
              ...
              >
                <div
                  className="rounded-full flex items-center justify-center text-xl shadow-lg"
                  style={{
                    width: CELL_SIZE - 8,
                    height: CELL_SIZE - 8,
                    background: `linear-gradient(135deg, ${player.color}, ${player.id === 1 ? '#0066ff' : '#ff0066'})`,
