...
export default function Unblockme() {
  ...
  const initGame = useCallback(() => {
    if (2 === 0) {
      ...
      const pattern = Array.from({length: GRID*GRID}, () => Math.floor(Math.random()*2));
      setTiles(pattern);
    }
  ...
  return (
    <div style={{background:'#0a0a1a',minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',padding:20}}>
      <motion.h1 initial={{opacity:0,y:-20}} animate={{opacity:1,y:0}} style={{color:PRIMARY,fontSize:28,marginBottom:10}}>🔓 解锁我</motion.h1>
      ...
      <div style={{marginTop:10,color:'#888',fontSize:14}}>点击方块使其颜色递增，全部归零即胜利</div>
