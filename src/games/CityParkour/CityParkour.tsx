...
import { CityParkourEngine, Obstacle } from './engine';

interface CityParkourProps {
...
export default function CityParkour({ onScoreUpdate, onGameOver, onExit }: CityParkourProps) {
  const [engine] = useState(() => new CityParkourEngine());
  ...
  const [buildings, setBuildings] = useState(() => engine.getState().buildings);
  const [isGameOver, setIsGameOver] = useState(false);
  const { record, updateScore } = useGameRecord(STORAGE_KEYS.CITY_PARKOUR);
  ...
  const getObstacleStyle = (obs: Obstacle) => {
    switch (obs.type) {
      case 'coin':
        return { backgroundColor: NEON_COLORS.gold, boxShadow: `0 0 20px ${NEON_COLORS.gold}` };
      case 'powerup':
        return { backgroundColor: NEON_COLORS.neonGreen, boxShadow: `0 0 20px ${NEON_COLORS.neonGreen}` };
      case 'car':
        return { backgroundColor: '#e74c3c', boxShadow: `0 0 15px #e74c3c80` };
      case 'cone':
        return { backgroundColor: '#f39c12', boxShadow: `0 0 10px #f39c1280` };
      default:
        return { backgroundColor: '#95a5a6', boxShadow: `0 0 10px #95a5a680` };
    }
  };
    ...
    <div className="flex flex-col items-center gap-4">
      <div className="flex items-center justify-between w-full max-w-[300px] px-4">
        ...
        <div className="text-center">
          ...
          <div className="text-xl font-bold" style={{ color: NEON_COLORS.neonCyan }}>{record.bestScore}m</div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4 mb-2">
        ...
        <div className="flex items-center gap-1">
          ...
          <span className="font-bold" style={{ color: NEON_COLORS.neonCyan }}>{speed.toFixed(1)}x</span>
        </div>
        {player.isShielded && (
          <div className="flex items-center gap-1">
      ...
      <div
        ...
        style={{
          ...
          background: `linear-gradient(180deg, #1a1a2e 0%, #16213e 30%, #1a1a2e 70%, #2d3436 100%)`,
      ...
      >
        ...
        <div
          ...
          style={{
            background: `repeating-linear-gradient(90deg, ${NEON_COLORS.neonCyan}40 0px, ${NEON_COLORS.neonCyan}40 40px, transparent 40px, transparent 80px)`
          }}
        />
        ...
        {Array(LANES - 1).fill(0).map((_, i) => (
          <div
            ...
            style={{
              left: (i + 1) * LANE_WIDTH,
              backgroundColor: `${NEON_COLORS.neonPink}40`
            }}
          />
        ))}
          ...
          <div
            ...
            style={{
              background: player.isShielded
                ...
                : `linear-gradient(135deg, ${NEON_COLORS.neonPink}, ${NEON_COLORS.neonCyan})`,
              boxShadow: player.isShielded
                ? `0 0 30px ${NEON_COLORS.neonGreen}`
                : `0 0 30px ${NEON_COLORS.neonPink}`,
      ...
      <div className="flex gap-4 mt-4">
        <motion.button
          ...
          style={{
            backgroundColor: NEON_COLORS.darkPurple,
            color: NEON_COLORS.neonCyan,
            boxShadow: `0 0 15px ${NEON_COLORS.neonCyan}40`,
            border: `2px solid ${NEON_COLORS.neonCyan}`
          }}
