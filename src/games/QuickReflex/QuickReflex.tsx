...
import { QuickReflexEngine } from './engine';
...
export default function QuickReflex() {
  const navigate = useNavigate();
  const [engine] = useState(() => new QuickReflexEngine());
  const [state, setState] = useState(() => engine.getState());
  const [gameStatus, setGameStatus] = useState<'idle' | 'playing' | 'gameover'>('idle');
  const [highScore, setHighScore] = useLocalStorage(STORAGE_KEYS.QUICK_REFLEX || 'quickreflex_highscore', 0);
  ...
  const render = useCallback(() => {
    ...
    if (state.phase === 'waiting' || state.phase === 'ready') {
      ...
      if (state.phase === 'waiting') {
        ...
        ctx.shadowBlur = 10;
        ctx.fillText('准备中...', centerX, centerY - 50);

        ctx.font = '24px Arial';
        ctx.fillStyle = '#888888';
        ctx.shadowBlur = 0;
        ctx.fillText('点击开始', centerX, centerY + 20);
      } else {
        ...
        ctx.fillText('等待变绿...', centerX, centerY - 50);

        ctx.font = '20px Arial';
        ctx.fillStyle = '#FF8800';
        ctx.shadowBlur = 0;
        ctx.fillText('不要提前点击!', centerX, centerY + 30);
      }
    }
    ...
    if (state.phase === 'click') {
      ...
      gradient.addColorStop(0.5, `rgba(0, 255, 0, ${pulse * 0.5})`);
      ...
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText('点击!', centerX, centerY);
    }

    if (state.phase === 'tooEarly') {
      const elapsed = Date.now() - state.phaseStartTime;
      const flash = Math.sin(elapsed / 50) > 0;

      ctx.fillStyle = flash ? 'rgba(255, 0, 68, 0.8)' : 'rgba(255, 0, 68, 0.3)';
      ctx.fillRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
      ...
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText('太快了!', centerX, centerY - 50);

      ctx.font = '24px Arial';
      ctx.fillStyle = '#FFFFFF';
      ctx.shadowBlur = 0;
      ctx.fillText('等待绿色出现再点击', centerX, centerY + 20);
    }
    ...
    if (state.phase === 'result' && state.reactionTime > 0) {
      ...
      ctx.fillText(`${state.reactionTime}ms`, centerX, centerY);

      if (state.bestTime < 9999 && state.reactionTime <= state.bestTime) {
        ctx.font = '24px Arial';
        ctx.fillStyle = '#FFD700';
        ctx.shadowColor = '#FFD700';
        ctx.fillText('本轮最佳!', centerX, centerY + 50);
      }
    }
    ...
    if (gameStatus === 'idle') {
      ...
      ctx.shadowColor = '#00FF00';
      ctx.shadowBlur = 20;
      ctx.fillText('⚡ 瞬间反应 ⚡', centerX, centerY - 40);

      ctx.font = '24px Arial';
      ctx.fillStyle = '#FFFFFF';
      ctx.shadowBlur = 10;
      ctx.fillText('点击开始游戏', centerX, centerY + 20);
    }
    ...
    if (gameStatus === 'gameover') {
      ...
      ctx.shadowBlur = 20;
      ctx.fillText('🏆 测试完成! 🏆', centerX, centerY - 100);

      ctx.font = 'bold 36px Arial';
      ctx.fillStyle = '#FFD700';
      ctx.shadowColor = '#FFD700';
      ctx.fillText(`最终得分: ${state.score}`, centerX, centerY - 40);

      ctx.font = '24px Arial';
      ctx.fillStyle = '#00FF00';
      ctx.shadowBlur = 10;
      ctx.fillText(`最快反应: ${state.bestTime}ms`, centerX, centerY + 10);

      ctx.fillStyle = '#A855F7';
      ctx.shadowColor = '#A855F7';
      ctx.fillText(`平均反应: ${state.averageTime}ms`, centerX, centerY + 50);
    }
  ...
  useEffect(() => {
    ...
    const animatePulse = () => {
      setPulseRadius(prev => (prev + 2) % 200);
  ...
  return (
    ...
    >
      ...
      >
        <div className="text-sm opacity-70" style={{ color: '#00FF00' }}>
          {gameStatus === 'idle' && '点击画布开始游戏'}
          {gameStatus === 'playing' && state.phase === 'waiting' && '点击开始测试'}
          {gameStatus === 'playing' && state.phase === 'ready' && '等待变绿...不要提前点击!'}
          {gameStatus === 'playing' && state.phase === 'click' && '快! 点击!'}
          {gameStatus === 'playing' && state.phase === 'tooEarly' && '太快了! 等变绿再点'}
          {gameStatus === 'playing' && state.phase === 'result' && '准备下一轮...'}
          {gameStatus === 'gameover' && '按"再来一局"重新开始'}
        </div>
      </motion.div>

      {gameStatus === 'playing' && state.bestTime < 9999 && (
        <motion.div
