...
export default function Juicebar() {
  ...
  const spawnObjects = useCallback(() => {
    ...
    for (let i = 0; i < 7; i++) {
      objectsRef.current.push({
        ...
        vx: (Math.random()-0.5)*2, vy: 1.9+Math.random(),
  ...
  return (
    <div style={{background:'#0a0a1a',minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',padding:20}}>
      <motion.h1 initial={{opacity:0,y:-20}} animate={{opacity:1,y:0}} style={{color:PRIMARY,fontSize:28,marginBottom:10}}>🍹 果汁吧</motion.h1>
