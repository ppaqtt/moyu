...
import { IdiomChainEngine, ChainItem, Idiom } from './engine';
...
const DIFFICULTIES = [
  { key: 'easy', label: '简单', subtitle: '45秒/回合', color: '#22c55e', time: 45 },
  { key: 'medium', label: '中等', subtitle: '30秒/回合', color: '#3b82f6', time: 30 },
  { key: 'hard', label: '困难', subtitle: '20秒/回合', color: '#f59e0b', time: 20 }
];

export default function IdiomChain() {
  const [difficulty, setDifficulty] = useState<Difficulty>('medium');
  const [engine, setEngine] = useState(() => new IdiomChainEngine('medium'));
  ...
  const startGame = (diff: Difficulty) => {
    const newEngine = new IdiomChainEngine(diff);
    setEngine(newEngine);
    setDifficulty(diff);
    ...
    setScore(0);
    setCombo(0);
    setTimeRemaining(newEngine.getTurnTime());
    setInput('');
    setMessage('');
    setWinner(null);
    setReason('');
  ...
  useEffect(() => {
    if (isUserTurn && phase === 'playing' && inputRef.current) {
  ...
  // Computer turn
  useEffect(() => {
    if (phase === 'playing' && !isUserTurn && !engine.isGameOver()) {
  ...
  }, [isUserTurn, phase, engine, difficulty, loadState]);
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.length >= 2) {
      const sugg = engine.getSuggestions(value);
      setSuggestions(sugg);
    } else {
      setSuggestions([]);
    }
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    if (!input.trim() || !isUserTurn || engine.isGameOver()) return;
  ...
  if (phase === 'menu') {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4"
        ...
        >
          <h1 className="text-6xl font-bold mb-4 bg-gradient-to-r from-red-400 via-yellow-500 to-red-400 bg-clip-text text-transparent">
            成语接龙
          </h1>
          <p className="text-gray-400 mb-8 text-xl">Idiom Chain Challenge</p>
          ...
          <div className="glass-card rounded-2xl p-8 max-w-md mx-auto mb-8">
            ...
            <ul className="text-gray-300 text-left space-y-2">
              <li>1. 电脑先出一个成语</li>
              <li>2. 你需要用上一个成语的最后一个字作为新成语的第一个字</li>
              <li>3. 在限定时间内完成接龙</li>
              <li>4. 连续接龙可获得连击加分!</li>
              <li>5. 电脑无法接龙时你获胜!</li>
            </ul>
        ...
        >
          ...
          <div className="glass-card rounded-2xl p-6 max-w-sm mx-auto mb-8">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <div className="text-sm text-gray-400">接龙次数</div>
  ...
  return (
    <div className="min-h-screen p-4 flex flex-col"
      ...
      >
        ...
        <div className={`glass-card px-6 py-3 rounded-xl ${timeRemaining <= 10 ? 'border-2 border-red-500' : ''}`}>
          <span className="text-gray-400 text-sm">剩余时间</span>
          <span className={`font-bold ml-2 text-xl ${timeRemaining <= 10 ? 'text-red-400' : 'text-cyan-400'}`}>
        ...
        <form onSubmit={handleSubmit} className="flex gap-3">
          <div className="flex-1 relative">
            <input
              ...
              disabled={!isUserTurn || engine.isGameOver()}
            ...
            {suggestions.length > 0 && (
              <div className="absolute bottom-full left-0 right-0 mb-2 bg-gray-800 rounded-xl border border-gray-600 overflow-hidden z-10">
          ...
          <button
            type="submit"
            disabled={!isUserTurn || !input.trim() || engine.isGameOver()}
