...
export default function Idlebreeding() {
  ...
  const spawnObjects = useCallback(() => {
    ...
    const colors = ['#FBBF24','#F59E0B','#22c55e','#3b82f6','#ffd700'];
    for (let i = 0; i < 7; i++) {
  ...
  return (
    <div style={{background:'#0a0a1a',minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',padding:20}}>
      <motion.h1 initial={{opacity:0,y:-20}} animate={{opacity:1,y:0}} style={{color:PRIMARY,fontSize:28,marginBottom:10}}>🐄 放置养殖</motion.h1>
      ...
      <div style={{marginTop:10,color:'#888',fontSize:14}}>方向键/WASD移动 | 点击收集资源，升级自动产出</div>
