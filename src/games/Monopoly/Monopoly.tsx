...
import { MONOPOLY_CONSTANTS, STORAGE_KEYS, NEON_COLORS } from '../../utils/constants';
import { MonopolyEngine, MonopolyState, Property } from './engine';

const { CANVAS_WIDTH, CANVAS_HEIGHT } = MONOPOLY_CONSTANTS;
...
export default function Monopoly() {
  ...
  const [engine] = useState(() => new MonopolyEngine());
  const [gameState, setGameState] = useState<MonopolyState>(() => engine.getState());
  const [highScore, setHighScore] = useLocalStorage<number>(STORAGE_KEYS.MONOPOLY, 0);
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const boardSize = CANVAS_WIDTH - 100;
      const margin = 50;
      const cellSize = boardSize / 10;
      const cornerSize = cellSize * 1.2;
      ...
      if (state.diceResult) {
        ...
        if (state.diceResult.isDouble) {
          ...
          ctx.fillText('🎲 双数!', CANVAS_WIDTH - 80, 55);
        }
      }
    };
  ...
  return (
    <div 
      className="min-h-screen flex flex-col items-center gap-6 py-8 px-4"
      style={{
