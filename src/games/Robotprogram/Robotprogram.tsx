...
export default function Robotprogram() {
  ...
  const spawnObjects = useCallback(() => {
    ...
    const colors = ['#22C55E','#10B981','#22c55e','#3b82f6','#ffd700'];
    for (let i = 0; i < 7; i++) {
  ...
  return (
    <div style={{background:'#0a0a1a',minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',padding:20}}>
      <motion.h1 initial={{opacity:0,y:-20}} animate={{opacity:1,y:0}} style={{color:PRIMARY,fontSize:28,marginBottom:10}}>🤖 机器人编程</motion.h1>
      ...
      <div style={{marginTop:10,color:'#888',fontSize:14}}>方向键/WASD移动 | 将指令方块按正确顺序排列，让角色到达终点</div>
