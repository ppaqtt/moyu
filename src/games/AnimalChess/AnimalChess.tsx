...
import { AnimalChessEngine, Player } from './engine';

const CANVAS_WIDTH = 385;
const CANVAS_HEIGHT = 385;
const CELL_SIZE = 55;

type GameStatus = 'idle' | 'playing' | 'gameover';

export default function AnimalChess() {
  const navigate = useNavigate();
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<AnimalChessEngine | null>(null);
  const animationFrameRef = useRef<number>(0);

  const [bestScore, setBestScore] = useLocalStorage<number>('animalchess_highscore', 0);
  ...
  useEffect(() => {
    engineRef.current = new AnimalChessEngine();
  ...
  return (
    ...
    >
      <motion.div
        className="glass-card rounded-3xl p-8 max-w-[500px] w-full"
        style={{
