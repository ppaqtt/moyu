...
export default function Subway() {
  ...
  const ballRef = useRef<Ball>({x:W/2, y:H-40, dx:3.8, dy:-3.8});
  ...
  const startGame = useCallback(() => {
    ...
    ballRef.current = {x:W/2, y:H-40, dx:3.8, dy:-3.8};
  ...
  useEffect(() => {
    ...
    const anim = setInterval(() => {
      ...
      if (ball.y > H + 20) {
        ...
        ball.x = W/2; ball.y = H-40; ball.dx = 3.8; ball.dy = -3.8;
      }
  ...
  return (
    <div style={{background:'#0a0a1a',minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',padding:20}}>
      <motion.h1 initial={{opacity:0,y:-20}} animate={{opacity:1,y:0}} style={{color:PRIMARY,fontSize:28,marginBottom:10}}>🚇 地铁跑酷</motion.h1>
