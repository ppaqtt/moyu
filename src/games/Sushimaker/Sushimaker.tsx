...
export default function Sushimaker() {
  ...
  const spawnObjects = useCallback(() => {
    ...
    for (let i = 0; i < 8; i++) {
      objectsRef.current.push({
        ...
        vx: (Math.random()-0.5)*2, vy: 2.2+Math.random(),
  ...
  return (
    <div style={{background:'#0a0a1a',minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',padding:20}}>
      <motion.h1 initial={{opacity:0,y:-20}} animate={{opacity:1,y:0}} style={{color:PRIMARY,fontSize:28,marginBottom:10}}>🍣 寿司师傅</motion.h1>
