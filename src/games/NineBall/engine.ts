...
export class NineBallEngine {
  ...
  private racksWon: number;
  private isBreakShot: boolean;
  private firstBallHit: number | null;
  ...
  private checkBallCollisions(): void {
    for (let i = 0; i < this.balls.length; i++) {
      ...
      for (let j = i + 1; j < this.balls.length; j++) {
        ...
        if (d < minDist && d > 0) {
          if (a.id === 0 && this.firstBallHit === null) {
            this.firstBallHit = b.id;
          } else if (b.id === 0 && this.firstBallHit === null) {
            this.firstBallHit = a.id;
          }
  ...
  private onBallsSettled(): void {
    const cueBall = this.balls.find(b => b.id === 0)!;
    const nineBall = this.balls.find(b => b.id === 9)!;

    let pocketedThisShot = this.pocketedBalls.filter(id => {
      return this.balls.find(b => b.id === id)?.pocketed && id !== 0;
    });

    let hitCorrectBall = this.firstBallHit === this.currentTarget || (this.firstBallHit !== null && this.firstBallHit < this.currentTarget);
    ...
    if (!hitCorrectBall && !this.isBreakShot && this.firstBallHit !== null) {
      this.foul = true;
      this.foulReason = '先击中目标球!';
      this.message = '犯规! 先击中目标球';
      this.consecutiveFouls++;
      this.ballInHand = true;
    ...
    if (nineBall.pocketed) {
      const lowestPocketed = this.getLowestPocketed();
      if (lowestPocketed === 9) {
        this.score += 100;
        this.racksWon++;
        this.isWin = true;
        this.isGameOver = true;
        this.message = `恭喜获胜! 赢得 {this.racksWon + 1} 局!`;
        this.gameState = 'gameover';
        return;
      } else {
        this.isGameOver = true;
        this.message = '9号球过早进袋!';
        this.gameState = 'gameover';
        return;
      }
    }

    const pocketedNonNine = this.pocketedBalls.filter(id => id !== 0 && id !== 9);
    if (pocketedNonNine.length > 0) {
      const lowest = Math.min(...pocketedNonNine);
      if (lowest < this.currentTarget) {
        this.currentTarget = lowest;
      }
    }

    if (pocketedNonNine.length > 0 || hitCorrectBall) {
      this.score += pocketedNonNine.length * 10 + (hitCorrectBall ? 5 : 0);
    }

    const remainingBalls = this.balls.filter(b => b.id !== 0 && !b.pocketed);
    if (remainingBalls.length === 0 || (remainingBalls.length === 1 && remainingBalls[0].id === 9)) {
      this.score += 100;
      this.racksWon++;
      this.message = '清台! +100分';
      this.newRack();
    } else {
      this.gameState = 'playing';
    }
  ...
  private getLowestPocketed(): number {
    const pocketed = this.pocketedBalls.filter(id => id !== 0);
    if (pocketed.length === 0) return 0;
    return Math.min(...pocketed);
  }
